...
################################################################################
# This file provides a class definition for a generation of RVEs with inclusions with positions and
# radii according to a list. The class inherits from the InclusionRVE class and extends it in order
# order to specify the remaining placeholder methods of the GenericModel. Methods to create the
# geometry, define refinement information and additional information for required boolean
...
from .InclusionRVE import \
    InclusionRVE  # generic RVE class definition (parent class)


###################################
# Define RandomInclusionRVE class #
###################################
class ListInclusionRVE(InclusionRVE):
    """Class definition for box-shaped RVEs with listed inclusions

    This class provides required information for box-shaped, RVEs with listed inclusions. It
    inherits from the InclusionRVE class and extends its attributes and methods to handle the
    inclusion placement as well as the definition of required boolean operations and physical
    groups.
    ...
    def __init__(self,
        ...
        super().__init__(size=size,
                         ...
                         inclusionAxis=inclusionAxis,
                         origin=origin,
                         periodicityFlags=periodicityFlags,
                         gmshConfigChanges=gmshConfigChanges)

        # plausibility checks for input variables
        if inclusionLists is None:
            raise TypeError(
                "Variable \"inclusionLists\" not set! The inclusionLists must be defined. Check your input data."
            )
        self.inclusionLists = inclusionLists
        """
        # update inclusion sets to start placement with biggest inclusions
        inclusionSets = np.atleast_2d(
            inclusionSets
        )  # type conversion for inclusionSets to be matrix-like
        inclusionSets = inclusionSets[inclusionSets[:, 0].argsort(
            axis=0
        )[::
          -1]]  # sort inclusionSets (descending) to start algorithm with biggest inclusions
...
############################################################################
    ...
    def defineGeometricObjects(self, placementOptions={}):
        ...
        # generate geometry
        self.addGeometricObject(
            self.domainType,
            group=self.domainGroup,
            origin=self.origin,
            size=self.size)  # add domain object to calling RVE
        self.placeInclusions(
            placementOptions)  # call internal inclusion placement routine
        for i in range(0,
                       np.shape(
                           self.inclusionInfo)[0]):  # loop over all inclusions
            if self.inclusionType in ["Sphere", "Circle"]:
                self.addGeometricObject(
                    self.inclusionType,
                    group=self.inclusionGroup,
                    center=self.inclusionInfo[i, 0:3],
                    radius=self.inclusionInfo[
                        i, 3])  # add inclusions to calling RVE object
            elif self.inclusionType == "Cylinder":
                self.addGeometricObject(
                    self.inclusionType,
                    group=self.inclusionGroup,
                    center=self.inclusionInfo[i, 0:3],
                    radius=self.inclusionInfo[i, 3],
                    axis=self.inclusionAxis
                )  # add inclusions to calling RVE object
    ...
    def defineBooleanOperations(self):
        ...
        """
        self.booleanOperations = [
            {  # first boolean operation (intersect domain group and inclusions group to get rid of parts that exceed the domain boundary)
                "operation":
                "intersect",  # -> intersection of "domain" and "inclusions"
                "object": self.domainGroup,  # -> use "domain" group as object
                "tool":
                self.inclusionGroup,  # -> use "inclusions" group as tool
                "removeObject":
                False,  # -> keep the object ("domain") after the boolean operation for further use
                "removeTool":
                True,  # -> remove the tool ("inclusions") after the boolean operation
                "resultingGroup": self.
                inclusionGroup  # -> assign the result of the boolean operation to the "inclusions" group (i.e. overwrite the group) for further use
            },
            {  # second boolean operation (cut domain with resulting inclusions to create holes within the domain where the inclusions are placed)
                "operation":
                "cut",  # -> cut "inclusions" (updated group) from "domain"
                "object": self.domainGroup,  # -> use "domain" group as object
                "tool":
                self.inclusionGroup,  # -> use "inclusions" group as tool
                "removeObject":
                True,  # -> remove the object ("domain") after the boolean operation
                "removeTool":
                False,  # -> keep the tool ("inclusions") after the boolean operation for further use
                "resultingGroup": self.
                domainGroup  # -> assign the result of the boolean operation to the "domain" group (i.e. overwrite the group)
            }
        ]
    ...
    def definePhysicalGroups(self):
        ...
        self.physicalGroups = [
            {
                "dimension":
                self.dimension,  # define dimension of the physical group
                "group": self.domainGroup,  # define name of the physical group
                "physicalNumber":
                1  # set physical number of the physical group
            },
            {
                "dimension":
                self.dimension,  # define dimension of the physical group
                "group":
                self.inclusionGroup,  # define name of the physical group
                "physicalNumber":
                2  # set physical number of the physical group
            },
            {
                "dimension":
                self.dimension - 1,  # define dimension of the physical group
                "group": "boundary",  # define name of the physical group
                "physicalNumber":
                3,  # set physical number of the physical group
            }
        ]
...
##########################################
    ...
    def placeInclusions(self, placementOptions):
        ...
        nSets = self.inclusionSets[:,
                                   1]  # amount of inclusions for the individual sets
        distBndSets = rSets * placementOptions[
            "minRelDistBnd"]  # distance of inclusions to the domain boundaries for the individual sets
        distIncSets = rSets * placementOptions[
            "minRelDistInc"]  # distance of inclusions to other inclusions for the individual sets

        # initialization
        totalInstancesSet = 0  # number of already set inclusion instances (inclusions and copies of them)
        incInfo = np.zeros(
            (4 * np.sum(nSets).astype(int), 6)
        )  # incInfo=[ [incCoords], rInc, mindDistBnd, mindDistInc] -> assume every inclusion to cut potentially two boundaries (i.e. to have three copies) for initialization
        self.placementInfo = np.zeros(
            np.shape(rSets)
        )  # information about placed inclusions for the individual sets

        # loop over all incSets
        relevantAxesFlags = np.zeros(
            (3, )
        )  # creaty auxiliary flag variable to indicate axes which are relevant for inclusion center calculation
        relevantAxesFlags[
            self.
            relevantAxes] = 1  # set relevantAxesFlags to 1 for the relevant axes
        for iSet in range(0, np.shape(rSets)[0]):
            ...
            while placedIncsForSet < nSets[
                    iSet] and nAttempts < placementOptions["maxAttempts"]:

                # calculate random coordinates for current inclusion (ensure 2D array for proper handling)
                thisIncInfo = np.atleast_2d(
                    np.r_[np.random.rand(3) * self.size * relevantAxesFlags,
                          rSets[iSet], distBndSets[iSet], distIncSets[iSet]])

                # check inclusion distance to boundaries
                acceptInc, distBnd = self._checkBndDistance(
                    thisIncInfo[0, :], self.relevantAxes)
                ...
                if any(self.periodicityFlags) == True:
                    thisIncInfo, thisIncInstances = self._periodicIncs(
                        thisIncInfo, distBnd)
                ...
                if totalInstancesSet > 0:  # only check if at least one inclusion has been placed
                    for i in range(
                            0, thisIncInstances
                    ):  # check i-th instance of current inclusion
                        acceptIncInstance = self._checkIncDistance(
                            thisIncInfo[i, :], incInfo[0:totalInstancesSet, :],
                            self.relevantAxes
                        )  # check if i-th instance is accepted
                    ...
                    if acceptInc == False:  # start with next attempt, if inclusion is not accepted
                        nAttempts += 1
                        if nAttempts == placementOptions[
                                "maxAttempts"]:  # user info, if not all inclusions of the current set could be placed
                            print(
                                "Could not place all inclusions for the current set. For r={0:.2f}, {1:.0f}/{2:.0f} have been placed."
                                .format(rSets[iSet], placedIncsForSet,
                                        nSets[iSet]))
                        continue
                    else:  # update incInfo and number of set inclusions/inclusion instances
                        incInfo[totalInstancesSet:totalInstancesSet +
                                thisIncInstances, :] = thisIncInfo
                        placedIncsForSet += 1
        ...
        incInfo[:, 0:3] = incInfo[:, 0:3] + np.atleast_2d(
            self.origin
        )  # translate center coordinates to match with defined origin of the RVE domain
        incInfo = incInfo[
            0:totalInstancesSet,
            0:4]  # incInfo=[M_x, M_y, M_z, R] with M: center points, R: radii
        ...
        # get inlusion radii and amounts for the individual sets
        self.inclusionSets = []
        for xyz, r in self.inclusionLists:
            self.inclusionSets.append([r, len(xyz)])

        self.inclusionSets = np.array(self.inclusionSets)
        rSets = np.asarray(self.inclusionSets[:, 0]
                           )  # radii of inclusion for the individual sets
        nSets = self.inclusionSets[:,
                                   1]  # amount of inclusions for the individual sets
        ...
        relevantAxesFlags[
            ...
            relevantAxes] = 1  # set relevantAxesFlags to 1 for the relevant axes
        for iSet in range(0, np.shape(rSets)[0]):
            xyz = self.inclusionLists[iSet][0]
            ...
            #        iSet] and nAttempts < placementOptions["maxAttempts"]:
            for xyz_j in xyz:
                # calculate coordinates for current inclusion (ensure 2D array for proper handling)
                thisIncInfo = np.atleast_2d(np.r_[xyz_j * relevantAxesFlags,
                                                  rSets[iSet],
                                                  distBndSets[iSet],
                                                  distIncSets[iSet]])
                ...
                acceptInc, distBnd = self._checkBndDistance(
                    thisIncInfo[0, :], self.relevantAxes)
                if acceptInc == False:  # go to next if not accepted
                    #nAttempts += 1
                    continue
                else:  # set number of required inclusion instances to 1, if inclusion is accepted
                    thisIncInstances = 1
    ...
    def saveIncInfo(self, file):
        ...
        """
        fileDir, fileName, fileExt = self._getFileParts(
            file, "Misc")  # split fileName into file parts
        with open(fileDir + "/" + fileName + fileExt,
                  "w") as incInfoFile:  # open file
            np.savetxt(incInfoFile,
                       self.inclusionInfo)  # save information to file
...
####################################################
    ...
    def _periodicIncs(self, thisIncInfo, distBnd):
        ...
        for i in range(
                0,
                np.shape(bndCuts)
            [1]):  # find all direction for which copies have to be generated
            if self.periodicityFlags[bndCuts[
                    1,
                    i]] == 1:  # only create periodic copy if current direction is marked as periodic
                thisIncCopies = cp.deepcopy(
                    thisIncInfo
                )  # initialize current copy with current inclusion data (deepcopy)
                thisIncCopies[:, bndCuts[1, i]] = thisIncCopies[:, bndCuts[
                    1, i]] + (-1)**bndCuts[0, i] * self.size[bndCuts[
                        1, i]]  # modify inclusion centers of copies
                thisIncInfo = np.r_[
                    thisIncInfo,
                    thisIncCopies]  # append copied inclusions to current inclusion data
                thisIncInstances = np.shape(thisIncInfo)[
                    0]  # update number of required inclusion instances
