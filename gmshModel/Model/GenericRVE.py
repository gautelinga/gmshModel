...
from .GenericModel import \
    GenericModel  # generic model class definition (parent class)
    ...
    def __init__(self,
        ...
        for varName, varValue in {
            ...
            if varValue is None:  # check if variable has a value
                raise TypeError(
                    "Variable \"{}\" not set! Check your input data.".format(
                        varName))
            elif len(np.shape(varValue)
                     ) > 1:  # check for right amount of array dimensions
                raise ValueError(
                    "Wrong amount of array dimensions for variable \"{}\"! For a cuboid RVE, the variable \"{}\" can only be one-dimensional. Check your input data."
                    .format(varName))
            elif not len(varValue) in [2, 3
                                       ]:  # check for right amount of values
                raise ValueError(
                    "Wrong number of values for variable \"{}\"! For a cuboid RVE, the variable \"{}\" has to have 2 or 3 values. Check your input data."
                    .format(varName))
            elif varName == "size" and np.count_nonzero(varValue) not in [
                    2, 3
            ]:  # check for right amount of non-zero values (size only)
                raise ValueError(
                    "Wrong number of non-zero values for variable \"{}\"! Only 2D/3D RVEs supported: the variable \"{}\" has to have 2 or 3 non-zero values. Check your input data."
                    .format(varName))

        # type conversion for input arguments
        size = np.asarray(size)  # type conversion for size to numpy array
        origin = np.asarray(
            origin)  # type conversion for origin to numpy array
        periodicityFlags = np.asarray(
            periodicityFlags
        )  # type conversion for periodicityFlags to numpy array

        # get dimension of RVE and correct potentially two-dimensional arrays
        dimension = np.count_nonzero(
            size)  # dimension equals number non-zero sizes
        if len(size) != 3:  # check if size is not a three-dimensional array
            size = np.r_[size, 0]  # -> append 0
        if len(origin
               ) != 3:  # check if origin is not a three-dimensional array
            newOrigin = np.zeros(3)  # -> create new three-dimensional array
            newOrigin[
                size !=
                0] = origin  # -> assign values of origin to non-zero dimensions of new array
...
#########################################################
    ...
    def setupPeriodicity(self):
        ...
        for iAx in range(0, self.dimension):  # loop over all axes
            if self.periodicityFlags[
                    iAx] == 1:  # -> check if current axis is supposed to be periodic
                assocEnts, affineMatrix = self._getAssociatedEntities(
                    iAx)  # ->-> get associated entities for the current axis
                self.gmshAPI.mesh.setPeriodic(
                    self.dimension - 1, assocEnts[1], assocEnts[0],
                    affineMatrix)  # ->-> set periodicity constraints
...
###################################################################
    ...
    def _getAssociatedEntities(self, axis):
        ...
        bboxRVE = np.r_[[self.origin], [
            self.origin + self.size
        ]]  # bounding box of the RVE: bboxRVE=[[minBBoxPoint],[maxBBoxPoint]]
        tol = 100 * self.getGmshOption(
            "Geometry.Tolerance"
        )  # tolerance for entity detection (factor 100 to find entities in wider bounding boxes)

        # calculate translation vector and affine transformation matrix
        transVec = np.zeros(
            (3, 1)
        )  # initialize translation vector for the current pair of boundary entities
        transVec[axis] = bboxRVE[1, axis] - bboxRVE[
            0,
            axis]  # update translation vector for the current pair of boundary entities
        affineMatrix = np.eye(
            4
        )  # initialize affine transformation matrix for current pair of boundary entities
        affineMatrix[0:3, [
            -1
        ]] = transVec  # update affine transformation matrix with translation information
        affineMatrix = affineMatrix.reshape(16).tolist(
        )  # convert affine transformation matrix to list for output
        ...
        for iSide in range(0, 2):  # loop over both sides (positive/negative)
            bboxBnd = cp.deepcopy(
                bboxRVE
            )  # -> initialze bounding box for the current boundary as copy of bboxRVE
            bboxBnd[1 - iSide, axis] = bboxRVE[
                iSide,
                axis]  # -> modify the coordinate of the relevant dimension to match the boundary under consideration
            bboxBnd = bboxBnd + np.array(
                [[-tol], [tol]])  # -> add tolerances to the bounding box

            # find entities on boundary under consideration
            entityTags = self.getIDsFromTags(
                self.gmshAPI.getEntitiesInBoundingBox(*bboxBnd[0], *bboxBnd[1],
                                                      self.dimension - 1))
            associatedEntities.append(entityTags)

        # sort associated entities of current pair to prevent errors
        associatedEntities = self._sortAssociatedEntities(
            associatedEntities, transVec)
    ...
    def _sortAssociatedEntities(self, associatedEntities, translationVector):
        ...
        for iEnt in range(0, nEntities):
            bboxesNeg[iEnt, :] = np.array(
                self.gmshAPI.getBoundingBox(dim, associatedEntities[0][iEnt]))
        ...
        for jEnt in range(0, nEntities):
            bboxEnt = np.array(
                self.gmshAPI.getBoundingBox(
                    dim, associatedEntities[1]
                    [jEnt]))  # find bounding box of current entity
            bboxesDiff = bboxesPos - bboxEnt  # calculate difference to all precomputed bounding boxes on positive side
            sortedIndices[jEnt] = np.argmin(
                np.linalg.norm(bboxesDiff, axis=1)
            )  # determine index of entity with minimum distance of actual and precomputed bounding boxes
